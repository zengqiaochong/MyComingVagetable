...
public class AliPaymentActivity extends BaseActivity {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					EventBus.getDefault().post(
							new EventMsg(OpCodes.PAY_ORDER_DONE,
									"支付完成"));
				} else {
	...
	private void initData() {
		...
		if (orderId == null || price == 0f) {
			Toast.makeText(mContext, "订单信息出错", Toast.LENGTH_SHORT).show();
			onBackPressed();
			return;
		}
		DecimalFormat df = new DecimalFormat("#0.00");
		String res = String.valueOf(df.format(price));
		tvPrice.setText(res + "元");

		btPay.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) { 
				 url = CommonAPI.BASE_URL
						+ String.format(CommonAPI.URL_PURCHARSE_WITH_ALIPAY, ShareUtil
								.getInstance(mContext).getUserId(), "", orderId,
								price, "userId:"+ShareUtil.getInstance(mContext).getUserId());
				Log.e("url", "支付订单接口： " + url);
//				pay(arg0);
			}
		});
				...
				if (bean.getState() == AccessNetState.Success) {
					try {
						...
						if (tbean.getRESULT_TYPE() == 1) {
							...
							EventBus.getDefault().post(
									new EventMsg(OpCodes.PAY_ORDER_ERROR,
											tbean.getRESULT_MSG()));
						}
					} catch (Exception ex) {
						EventBus.getDefault().post(
					...
					EventBus.getDefault().post(
							new EventMsg(OpCodes.PAY_ORDER_ERROR, bean
									.getState()));
				}
			}
		}).start();
	...
	public void onEventMainThread(EventMsg msg) {
		...
		case OpCodes.PAY_ORDER_DONE:
			Toast.makeText(mContext, "订单处理成功", Toast.LENGTH_SHORT).show();
			onBackPressed();
			break;
		case OpCodes.PAY_ORDER_ERROR:
	...
	public void pay(View v) {
		...
		String orderInfo = getOrderInfo("菜来了商品", ShareUtil.getInstance(mContext).getUserId()+";菜来了商品支付",
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(AliPaymentActivity.this);
				// 调用支付接口，获取支付结果

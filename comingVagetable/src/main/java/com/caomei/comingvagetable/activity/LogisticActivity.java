...
public class LogisticActivity extends BaseActivity {
	...
	private void requestOrderLogistic() {
		final String url=CommonAPI.BASE_URL+String.format(CommonAPI.GET_DELIVER_INFO_BY_ORDER_ID,"0","100000000","0",orderId,ShareUtil.getInstance(mContext).getUserId(),"0","","");
		Log.e("url", "获取订单物流信息的接口  "+url);
		new Thread(new Runnable() { 
			...
			public void run() {
				...
				if(bean.getState()==AccessNetState.Success){
					...
					try {
						logisticBean=new Gson().fromJson(bean.getResult(), LogisticBean.class);
						EventBus.getDefault().post(new EventMsg(OpCodes.GET_LOGISTIC_INFO_DONE, null));
					} catch (Exception e) {
						EventBus.getDefault().post(new EventMsg(OpCodes.GET_LOGISTIC_INFO_ERROR, "物流信息解析出错"));
						
					}
				}else{
					EventBus.getDefault().post(new EventMsg(OpCodes.GET_LOGISTIC_INFO_ERROR, bean.getResult()));
				}
			}
		}).start();
	}
...
//			 dataSet.addAll(logisticBean.getData());
			 ...
			 tvOrderNo.setText(orderData.getOrderNO());
